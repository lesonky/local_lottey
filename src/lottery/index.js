...
function initAll() {
  ...
  // 读取当前已设置的抽奖结果
  basicData.leftUsers = localdata.leftUsers;
  basicData.luckyUsers = localdata.luckyData;
  ...
  basicData.users = localdata.users;
 ...
 * 创建图片元素
 */
function createImgElement(className, text) {
...
function createCard(user, isBold, id, showTable) {
  ...
  element.appendChild(createImgElement("user-image", user.image));

  element.appendChild(createElement("user-name", user.name));
  return element;
}
...
function selectCard(duration = 600) {
  ...
  let text = currentLuckys.map(item => item.name);
  addQipao(
...
function changeCard(cardIndex, user) {
  ...
  card.innerHTML = `<div class="user-name">${user.name
    }</div><img class="user-image" src="${user.image}"></img>`;
}
